    ...
    public class UserController : ControllerBase
    {
        ...
        public IActionResult Register([FromBody] UserRegisterDto user)
        {
            try
            {
                ...
                if (findUser)
                    return response.ErrorResponse($"Ya existe una cuenta con ese correo electrónico, intente de nuevo", user);
                ...
                return response.SuccessResponse("Usuario registrado, revise su bandeja de entrada para activar su cuenta", userResponse);
            }
            catch (Exception ex)
            {
                return response.ErrorResponse($"{ex.Message}", user);
            }
        }
        ...
        public IActionResult Authenticate([FromBody] AuthRequestDto auth)
        {
            ...
            if (user == null)
                return response.ErrorResponse($"Usuario no encontrado {auth.Email}", auth);

            if (!_userService.VerifyPassword(auth.Password, user.PasswordHash, user.PasswordSalt))
                return response.ErrorResponse($"Contraseña incorrecta", auth);

            if (user.State == StateEnum.DISABLED)
                return response.ErrorResponse($"Usuario inhabilitado {auth.Email}", auth);

            if (user.EmailVerified == StateEnum.DISABLED)
                return response.ErrorResponse($"Necesita activar su cuenta, revise su email por favor", auth);

            UserLoggedDto userLogged = _mapper.Map<UserLoggedDto>(user);
            userLogged.Token = _userService.GetToken(user.Id, user.Email);
            return response.SuccessResponse("Inicio de sesión exitoso, bienvenido", userLogged);
        }
        ...
        public IActionResult AuthenticatePlanWise([FromBody] AuthRequestDto auth)
        {
            ...
            if (user is null)
                return response.ErrorResponse($"No se encontro ningun usuario con: ", auth);
            ...
            return response.SuccessResponse("Inicio de sesión exitoso, bienvenido", sincronizarData);
        }
        ...
        public IActionResult PlanByClient(string email)
        {
            ...
            if (user is null)
                return response.ErrorResponse($"No se encontro ningun usuario con: ", email);
            ContractedPlans? contratosPlan = _context.ContractedPlans
                                                     .Include(x => x.Client)
                                                     .Include(x => x.Plan)
                                                     .Where(x => x.UserId.ToString() == user.Id.ToString()).FirstOrDefault();
        ...
        public IActionResult ResetPassword([FromBody] ResetPasswordDto reset)
        {
            if (_userService.FindUserByEmail(reset.Email) is null)
                return response.ErrorResponse($"Correo electrónico no registrado", reset);
            ...
            return response.SuccessResponse("Éxito, revise su bandeja de entrada para restaurar su contraseña", reset);
        }
        ...
        public IActionResult RestorePassword([FromBody] RestorePasswordDto restore)
        {
            ...
            if (ID is null)
                return response.ErrorResponse("El link enviado es inválido", restore);
            ...
            _context.SaveChanges();
            return response.SuccessResponse("Su contraseña se ha actualizado", userDto);
        }
        ...
        public IActionResult UpdateUser([FromBody] UpdateClienteDto userUpdate)
        {
            ...
            if (idUser is null)
                return response.SuccessResponse("No se encontró el usuario", userUpdate);
            ...
            if (!user.Email.Equals(userUpdate.Email))
            {
                if (_userService.FindUserByEmail(userUpdate.Email) is not null)
                    return response.ErrorResponse("Intente con un correo electrónico diferente", userUpdate);
            ...
            _context.Users.Update(updated);
            _context.SaveChanges();
            return response.SuccessResponse(message, userDto);
        }
        ...
        public IActionResult VerifyEmail(CustomerDto customer)
        {
            ...
            if(userId is not null)
            {
                ...
                if(user.ContractedPlans.Count > 0)
                    return response.ErrorResponse("Ya tiene un plan registrado, si desea comprar otro plan, debe crear otro usuario con diferente correo electrónico", customer.Email);
            }
            ...
            if(user is not null && user.ContractedPlans.Count > 0)
                return response.ErrorResponse("Ya tenemos registrado ese correo electronico, inicie sesión o intente con un correo diferente", customer.Email);
            ...
            return response.SuccessResponse("Ok", customer.Email);
        }
        ...
        public IActionResult WhoIAm()
        {
            ...
            return response.SuccessResponse("Ok", user);
        }
        ...
        public IActionResult UpdatePassword([FromBody] UpdatePasswordDto data)
        {
            ...
            if (user is null) return response.ErrorResponse("No se encontró el usuario", user);

            if (!_userService.VerifyPassword(data.Password, user.PasswordHash, user.PasswordSalt))
                return response.ErrorResponse("Contraseña actual incorrecta", user);
            UserDto userDto = _userService.UpdatePassword(data, user);
            _context.SaveChanges();
            return response.ErrorResponse("Contraseña actualizada correctamente", userDto);
        }
